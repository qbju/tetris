...
- 通常、グレースケール、反転、KONAMI、01、GOLDENのカラーテーマ
- 矢印キー操作とWASD操作の切り替え
- 落下速度、BGM音量、SE音量、デバッグ表示、CLOCK、Ghostの設定
...
設定は3ページ構成です。MUSICとCLOCKは対応する隠し実績の解除後に利用できます。

- Page 1: COLOR、GRAVITY、CONTROL
- Page 2: BGM VOL、SE VOL、DEBUG
- Page 3: MUSIC、CLOCK、GHOST

隠し実績以外を14個解除するとGOLDENパレット、19個すべて解除するとANY音楽と譜面エディタが解禁されます。

左右キーで値を変更します。Enterで次の行へ進み、`NEXT PAGE`または`SAVE BACK`でEnterを押すとページ移動・保存を実行します。CLOCKとGHOST設定もTinyFSへ保存されます。

## ANY音楽エディタ

隠し実績を除く19実績を解除すると、MUSIC設定で`KOROBEINIKI / ANY`を選択できます。ANYを選択してEnterを押すと、32音の五線譜風エディタを開きます。
...
| `S` | 編集中の32音を先頭から試聴 |
| `R` | コロブチカ先頭32音へリセット |
| `Enter` | TinyFSへ保存して設定へ戻る |
| `Esc` | 変更を保存せず設定へ戻る |

ANY譜面は1音あたりPIT分周値と音長を持ち、sector 14の世代番号・チェックサム付き二重スロットへ保存されます。
## 実績
...
解除通知はゲーム画面右下へ表示され、約3秒後に消えます。通常実績を5個解除するとグレースケール、9個解除すると300 ms以下の落下速度が解禁されます。隠し実績以外を14個解除するとGOLDEN、19個解除するとANYと音楽エディタが解禁されます。隠し実績ではKONAMI・01テーマやCLOCKなどが解禁されます。
