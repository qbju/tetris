...
- 非同期PCスピーカーSEとコロブチカのループ再生（同時発生時はSE優先）
...
ホームメニューにはSTATISTICSの下にACHIEVEMENTSがあり、左右キーで通常／隠し実績ページを切り替えます。解除時はゲーム画面右下へ通知を表示し、状態はTinyFSセクタ13のチェックサム付き二重スロットへ保存します。

通常実績5個でグレースケール、9個ですべての落下速度を解禁します。隠し実績ではKONAMIパレット、白黒01パレット、RTCベースのゲーム時計、条件付きGLITCH／BUILD INFOを解禁します。

`IMPOSSIBLE`は[kernel/parts/00_platform.inc.py](kernel/parts/00_platform.inc.py)の`impossible_enabled`を1へ直接変更してビルドした場合だけ解除されます。
## タイマー・音声・乱数

PITチャンネル0はラップ回数だけでなく、カウンタ差分と端数を累積して10ms周期へ変換します。余った経過時間は次回へ繰り越すため、描画時間による落下・入力タイマーのドリフトを抑えます。

効果音は待機ループを使わない非同期シーケンサーです。BGMとSEが重なった場合はSEがPCスピーカーを占有し、終了後にBGMを新しい音符から再開します。

7-bagの乱数シードはゲーム開始時に、PIT現在値、起動後の経過時間、直近のキー入力を混合して初期化します。
## 現在の制約
